    ...
    container.innerHTML = foods.map(f => `
      <div class="food-card">
        ...
        <div class="catalog-result-info">
          ...
          <div class="catalog-result-source">${formatSource(f.source, f.components)}${f.barcode ? ' · ' + f.barcode : ''}</div>
        </div>
        ...
        <div class="catalog-result-item">
          ...
          <div class="catalog-result-info">
            ...
            <div style="font-size:0.7rem;color:var(--color-text-secondary);margin-top:2px">
              ${formatSource(p.source)}${p.barcode ? ' · ' + p.barcode : ''}${p.nutriscore ? ' · Nutriscore ' + p.nutriscore.toUpperCase() : ''}
            </div>
          </div>
          ${action}
        </div>`;
